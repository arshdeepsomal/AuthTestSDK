...
class SessionManagerDelegateTest {
    ...
    fun `getSession returns stored value`() {
        ...
        coEvery { sessionPreferences.getSessionData() } returns flowOf(expected)

        val delegate = SessionManagerDelegate(context, sessionPreferences)
    ...
    fun `hasTokenExpired returns true when authorization is empty`() {
        ...
        coEvery { sessionPreferences.getSessionData() } returns flowOf(emptySession)
    ...
    fun `hasTokenExpired parses jwt expiration`() {
        ...
        coEvery { sessionPreferences.getSessionData() } returns flowOf(session)
