...
internal class ONEAuthDelegateTest {
    ...
    fun setup() {
        ...
        mockkConstructor(OneAuthClient::class)
        mockkConstructor(TwoAuthClient::class)
    }
    ...
    fun `login publishes launch intent`() = runBlocking {
        every { anyConstructed<OneAuthClient>().buildLoginIntent() } returns Intent("login")

        val delegate = ONEAuthDelegate(context, sessionManager, oneConfig, twoConfig)
        delegate.login()
    ...
    fun `logout clears session and emits success`() = runBlocking {
        ...
        coEvery { anyConstructed<TwoAuthClient>().logout(any()) } returns Result.success(Unit)
    ...
    fun `refreshToken saves renewed session`() = runBlocking {
        ...
        every { sessionManager.getSession() } returns sessionData
        coEvery { anyConstructed<TwoAuthClient>().renewToken(any()) } returns Result.success(
